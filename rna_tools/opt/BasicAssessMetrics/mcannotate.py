...
class MCAnnotate:
    ...
    def load(self, pdb_file, mc_dir, force_to_overwrite=True):
        """Load a PDB file.

        Defines the annotation file
        Check if the annotation file exists, if not, MC-Annotate is executed.
        Parse the annotation file

        Args:
            pdb_file (str): the path of the file
            mc_dir (???): ???
            force_to_overwrite (bool): if True, will force MC-Annotate execution even if the file exists
        ...
        if force_to_overwrite:
            # create a new annotation file
            if debug: print('force to generate a new file')
            ...
            #os.system(cmd)
            o = subprocess.Popen(
                cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=d)
            out = o.stdout.read().strip().decode()
            print(out)
            err = o.stderr.read().strip().decode()
            if err:
                print(err)
        elif not os.path.isfile(self.mc_file):
            # create a new annotation file
            if debug: print('mcout does not exists, generate a new file')
            cmd = "%s %s > %s" %(MCAnnotate_bin, pdb_file, self.mc_file)
            if debug: print(cmd)
            o = subprocess.Popen(
...
if __name__ == '__main__':
    ...
    mca.load( "/Users/magnus/work-src/rna-tools/rna_tools/opt/BasicAssessMetrics/example/14_solution_0.pdb", "/Users/magnus/work-src/rna-pdb-tools/rna_tools/opt/BasicAssessMetrics" )
